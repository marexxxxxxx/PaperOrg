...
departmentsRouter.patch('/:id', (req, res) => {
  ...
  if (name !== undefined) db.run('UPDATE departments SET name = ? WHERE id = ?', [name, req.params.id])
  if (description !== undefined) db.run('UPDATE departments SET description = ? WHERE id = ?', [description, req.params.id])
