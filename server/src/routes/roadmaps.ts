...
roadmapsRouter.patch('/:id', (req, res) => {
  ...
  if (name !== undefined) db.run('UPDATE roadmaps SET name = ? WHERE id = ?', [name, req.params.id])
  if (description !== undefined) db.run('UPDATE roadmaps SET description = ? WHERE id = ?', [description, req.params.id])
  if (start_date !== undefined) db.run('UPDATE roadmaps SET start_date = ? WHERE id = ?', [start_date, req.params.id])
  if (end_date !== undefined) db.run('UPDATE roadmaps SET end_date = ? WHERE id = ?', [end_date, req.params.id])
  if (status !== undefined) db.run('UPDATE roadmaps SET status = ? WHERE id = ?', [status, req.params.id])
