...
milestonesRouter.get('/', (req, res) => {
  ...
  while (stmt.step()) {
    ...
    milestones.push({
      ...row,
      ticket_ids: JSON.parse(row.ticket_ids as string || '[]')
    })
  }
  stmt.free()
...
milestonesRouter.patch('/:id', (req, res) => {
  ...
  if (name !== undefined) db.run('UPDATE milestones SET name = ? WHERE id = ?', [name, req.params.id])
  if (description !== undefined) db.run('UPDATE milestones SET description = ? WHERE id = ?', [description, req.params.id])
  if (target_date !== undefined) db.run('UPDATE milestones SET target_date = ? WHERE id = ?', [target_date, req.params.id])
  if (status !== undefined) db.run('UPDATE milestones SET status = ? WHERE id = ?', [status, req.params.id])
  if (ticket_ids !== undefined) db.run('UPDATE milestones SET ticket_ids = ? WHERE id = ?', [JSON.stringify(ticket_ids), req.params.id])
