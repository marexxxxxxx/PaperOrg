...
routingRulesRouter.patch('/:id', (req, res) => {
  ...
  if (source_department_id !== undefined) db.run('UPDATE routing_rules SET source_department_id = ? WHERE id = ?', [source_department_id, req.params.id])
  if (target_role_id !== undefined) db.run('UPDATE routing_rules SET target_role_id = ? WHERE id = ?', [target_role_id || null, req.params.id])
  if (target_department_id !== undefined) db.run('UPDATE routing_rules SET target_department_id = ? WHERE id = ?', [target_department_id || null, req.params.id])
