  ...
  `);
    ...
    const agents = result[0].values.map(row => {
        ...
        columns.forEach((col, i) => {
            obj[col] = col === 'capabilities' ? JSON.parse(row[i] || '[]') : row[i];
        });
        return obj;
    });
    res.json(agents);
...
agentsRouter.patch('/:id', (req, res) => {
    ...
    stmt.free();
    if (name !== undefined)
        db.run('UPDATE agents SET name = ? WHERE id = ?', [name, req.params.id]);
    if (role_id !== undefined)
        db.run('UPDATE agents SET role_id = ? WHERE id = ?', [role_id || null, req.params.id]);
    if (parent_id !== undefined)
        db.run('UPDATE agents SET parent_id = ? WHERE id = ?', [parent_id || null, req.params.id]);
    if (status !== undefined)
        db.run('UPDATE agents SET status = ? WHERE id = ?', [status, req.params.id]);
    if (capabilities !== undefined)
        db.run('UPDATE agents SET capabilities = ? WHERE id = ?', [JSON.stringify(capabilities), req.params.id]);
    if (description !== undefined)
        db.run('UPDATE agents SET description = ? WHERE id = ?', [description, req.params.id]);
    if (avatar_url !== undefined)
        db.run('UPDATE agents SET avatar_url = ? WHERE id = ?', [avatar_url, req.params.id]);
    if (department_id !== undefined)
        db.run('UPDATE agents SET department_id = ? WHERE id = ?', [department_id || null, req.params.id]);
