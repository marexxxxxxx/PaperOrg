...
rolesRouter.get('/', (req, res) => {
    ...
    const roles = result[0].values.map(row => {
        ...
        columns.forEach((col, i) => {
            obj[col] = col === 'permissions' ? JSON.parse(row[i] || '[]') : row[i];
        });
        return obj;
    });
    res.json(roles);
...
rolesRouter.post('/', (req, res) => {
    ...
    const id = uuidv4();
    const db = getDb();
    db.run(`INSERT INTO roles (id, name, permissions, tier, color) VALUES (?, ?, ?, ?, ?)`, [id, name, JSON.stringify(permissions), tier, color]);
    saveDb();
...
rolesRouter.patch('/:id', (req, res) => {
    ...
    const db = getDb();
    if (name !== undefined)
        db.run('UPDATE roles SET name = ? WHERE id = ?', [name, req.params.id]);
    if (permissions !== undefined)
        db.run('UPDATE roles SET permissions = ? WHERE id = ?', [JSON.stringify(permissions), req.params.id]);
    if (tier !== undefined)
        db.run('UPDATE roles SET tier = ? WHERE id = ?', [tier, req.params.id]);
    if (color !== undefined)
        db.run('UPDATE roles SET color = ? WHERE id = ?', [color, req.params.id]);
    saveDb();
    const stmt = db.prepare('SELECT * FROM roles WHERE id = ?');
    stmt.bind([req.params.id]);
    stmt.step();
